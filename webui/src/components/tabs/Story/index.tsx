...
export function StoryTab() {
  ...
  const volumeLabel = (volume = 1) => `0.${Math.min(9, Math.max(1, volume))}`;
  ...
  const create = async () => {
    ...
    try {
      const created = await storyApi.createStory({
        title, premise, genre, model_name: model, max_scenes: 12,
  ...
  return <div className="space-y-lg">
    ...
    <Card title="📖 Fortlaufende Geschichte" icon={BookOpen}>
      ...
      <div className="flex gap-sm mt-md flex-wrap">
        ...
        {story && story.status !== 'running' && (story.volume || 1) < 9 && <Button onClick={start} icon={Play}>{story.status === 'finished' ? `Band ${volumeLabel((story.volume || 1) + 1)} erzeugen` : `Band ${volumeLabel(story.volume)} weiter vertonen`}</Button>}
        {story?.status === 'finished' && (story.volume || 1) >= 9 && <span className="px-md py-sm text-accent-cyan">Serie mit Band 0.9 abgeschlossen</span>}
