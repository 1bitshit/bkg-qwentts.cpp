...
start_server() {
    ...
    if [ -f "./build/bin/bkg-qwen-tts-server" ]; then
        ...
        echo -e "${RED}[!] Fehler: Die ausführbare Server-Datei wurde nicht gefunden! Bitte kompiliere das Projekt zuerst (Option 6).${NC}"
        exit 1
    fi
}

# =========================================================================
# STRUKTURIERTE LOGIK-WEICHE (VON DIR ANGEPASST FÜR OPTION 6 & 7)
# =========================================================================
case $OPTION in
    1) 
        install_basis_tools
        sync_submodules
        build_project "false" # Baut direkt CPU, um shaderc-Fehler zu ignorieren
        download_model 
        ;;
    2) 
        install_basis_tools 
        ;;
    3) 
        echo -e "${YELLOW}[!] Grafik-SDKs übersprungen (Fehlendes shaderc umgangen)${NC}" 
        ;;
    4) 
        sync_submodules 
        ;;
    5) 
        ...
        ;;
    6) 
        build_project "false" # OPTION 6: Startet den reinen CPU-Build
        ;;
    7) 
        download_model        # OPTION 7: Startet den reinen Modell-Download
        ;;
    8) 
        start_server 
        ;;
    9) 
        ...
        ;;
    *) 
        echo -e "${RED}Ungültige Auswahl. Bitte starte das Skript neu.${NC}"
        exit 1 
        ;;
esac
