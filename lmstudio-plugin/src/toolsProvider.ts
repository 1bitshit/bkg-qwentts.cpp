...
const BASE_URL = process.env.BKG_QWENTTS_URL ?? "http://127.0.0.1:8010";
...
export async function toolsProvider(ctl: ToolsProviderController) {
  ...
  tools.push(tool({
    ...
    implementation: async ({ text, voice, language, file_name, seed, temperature }) => {
      ...
      const safeName = file_name.endsWith(".wav") ? file_name : `${file_name}.wav`;
